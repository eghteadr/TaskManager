...
<div class="grid min-h-screen w-full overflow-hidden lg:grid-cols-[280px_1fr]">
    ...
    <div class="flex flex-col">
        <header class="flex h-14 items-center gap-4 bg-white shadow px-6">
            <h1 class="text-xl font-bold">ویرایش تسک</h1>
        </header>

        <main class="flex flex-col gap-6 p-6">

            <section id="edit-task" class="bg-white shadow p-6 rounded-lg" style="direction: rtl">
                <h2 class="text-lg font-bold mb-4">ویرایش تسک</h2>
                ...
                <form action="{{ route('project.update', $project->id) }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="task-desc" name="description" rows="4"
                                  ...
                                  placeholder="توضیحات تسک را وارد کنید">{{ old('description', $project->description) }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md">ذخیره تغییرات</button>
                </form>
            </section>
